...
void CGridListCtrlEx::OnContextMenuHeader(CWnd* pWnd, CPoint point, int nCol)
{
	...
	case 0:
		break;
	case 1:
		OpenColumnEditor(nCol);
		break;
	case 2:
		OpenColumnPicker();
		break;
	case 3:
		ResetColumnDefaultState();
		break;
	default:
	{
		int nCol = nResult - 4;
...
DROPEFFECT CGridListCtrlEx::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
	...
	if (GetItemCount() > 0)
	{
		...
		else if (pt.y > gridRect.bottom - cellRect.Height())
		{
			Sleep(100);
...
BOOL CGridListCtrlEx::ResizeColumns()
{
	CRect rect;
	int adjustWidth, lastWidth;
	int adjustCols = 0;
	GetClientRect(&rect);
	...
	{
		CGridColumnTrait* pTrait = GetColumnTrait(iCol);
		int colWidth = GetColumnWidth(iCol);
		if (pTrait->HasMetaFlag(GCSF_FIXED))
		{
			adjustWidth -= GetColumnWidth(iCol);
		}
		else
		{
			adjustCols++;
	...
	if (adjustCols <= 0) return FALSE;

	adjustWidth /= adjustCols;
	lastWidth = adjustWidth + (adjustWidth % adjustCols);
	...
	for (int iCol = 1; iCol < GetColumnCount(); iCol++)
	{
		...
		if (!pTrait->HasMetaFlag(GCSF_FIXED))
		{
			int colWidth = adjustWidth;
			if (--adjustCols == 0)
			{
				colWidth = lastWidth;
				SetColumnWidth(iCol, colWidth);
				break;
			}
			else
				SetColumnWidth(iCol, colWidth);
